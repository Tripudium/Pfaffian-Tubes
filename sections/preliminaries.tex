...
References for the material in this section are~\cite{khovanskii1991fewnomials, zell2003quantitative, gabrielov2004complexity}. We assume throughout that $n\in \N_{>0}$. Pfaffian functions are defined relative to an open domain $\cU\subseteq\R^n$; unless a domain is specified we take $\cU=\R^n$, which covers the activation functions of interest, as these are Pfaffian on all of $\R^n$. A proper subdomain is needed only once, in the proof of Proposition~\ref{prop:single-layer-ideg}.
