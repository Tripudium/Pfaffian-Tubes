...
Let $M \subset B(p, \sigma)$ be a compact Riemannian submanifold of $\R^n$ of dimension $m$, possibly with boundary, and set $c=n-m$. Then for any $\varepsilon > 0$ we have 
\begin{equation*}
...
\begin{remark}
Note that the constant involves both $\beta$ and $\overline{\beta}=\max\{\beta,2\}$: the bare $\beta$ is the degree of $f$, while $\overline{\beta}$ is forced by the codimension-two boundary $V\cap S^{n-1}(p,\sigma+\varepsilon)$ arising in the proof, whose defining sphere has degree~$2$.
\end{remark}
...
\end{equation*}
Fix $\varepsilon>0$ and assume first that $\sigma+\varepsilon$ is a regular value of the map $x\mapsto\|x-p\|$ on $V$, which holds for almost all $\sigma>0$ by Sard's theorem; the remaining values of $\sigma$ are handled by a limiting argument at the end of the proof. Then $M = V \cap B(p, \sigma + \varepsilon)$ is a compact manifold with boundary, and we notice that
\begin{equation*}
T(V, \varepsilon) \cap B(p, \sigma) \subset T(M \setminus \partial M, \varepsilon) \cup T(\partial M, \varepsilon),
\end{equation*}
where $\partial M=M\cap S^{n-1}(p, \varepsilon+\sigma)$ is the sphere of radius $\varepsilon+\sigma$ centred at $p$.
It is therefore enough to bound the two tubular neighbourhoods on the right-hand side.

Since $M \setminus \partial M \subset V$ and $\dim(M \setminus \partial M) = \dim V$, we have $\md_i(M\setminus \partial M) \leq \md_i(V)$. Applying Theorem~\ref{thm:tube_bound} to the compact manifold-with-boundary $M\subset B(p,\sigma+\varepsilon)$ (see Remark~\ref{rem:tube_bound_boundary}) and using $\md_i(M\setminus\partial M)\le\md_i(V)$, we obtain
\begin{equation*}
  \vol T(M \setminus \partial M, \varepsilon)  \leq 2 \omega_n (\sigma+\varepsilon)^n\sum_{i=0}^{n-1} \binom{n}{i+1} \cdot \md_i(V)\cdot \left(\frac{\varepsilon}{\sigma+\varepsilon}\right)^{i+1}.
\end{equation*}
Since $\partial M$ has codimension $2$, we get, in addition,
\begin{equation*}
  \vol T(\partial M, \varepsilon) \leq 2 \omega_n (\sigma+\varepsilon)^{n}\sum_{i=1}^{n-1} \binom{n}{i+1} \cdot \md_{i-1}(\partial M)\cdot \left(\frac{\varepsilon}{\sigma+\varepsilon}\right)^{i+1}.
\end{equation*}
Set
\begin{equation*}
...
\end{align*}
Dividing both sides by $\vol B(p, \sigma) = \omega_n \sigma^n$, substituting $j=i+1$ and using the binomial theorem
%and setting $a=\alpha+\overline{\beta}$ and $t=\varepsilon/\sigma$, we get
%\begin{equation*}
...
%\end{equation*}
%Recalling $C_{\alpha,\beta,s,n}=6K$, $a=\alpha+\overline{\beta}$, and $t=\varepsilon/\sigma$ 
gives the claimed bound for every $\sigma$ such that $\sigma+\varepsilon$ is a regular value of $x\mapsto\|x-p\|$ on $V$, which by Sard's theorem is a dense set of $\sigma>0$. It remains to remove this genericity assumption. Both sides of the inequality are continuous in $\sigma$: the right-hand side is a polynomial in $\varepsilon/\sigma$, while the left-hand side equals $\vol\bigl(T(V,\varepsilon)\cap B(p,\sigma)\bigr)/(\omega_n\sigma^n)$, which is continuous because $\sigma\mapsto\vol\bigl(T(V,\varepsilon)\cap B(p,\sigma)\bigr)$ is. Since the bound holds on a dense set of $\sigma$ and both sides are continuous, it holds for all $\sigma,\varepsilon>0$, completing the proof.
\end{proof}

\begin{remark}\label{rem:prob_bound_simple}
Discarding the non-positive term $-(1+\varepsilon/\sigma)^n$ gives the simpler bound
\begin{equation*}
 \prob\{ d(X, V) \le \varepsilon \} \leq C_{\alpha,\beta,s,n}\left(1+(\alpha+\overline{\beta}+1)\frac{\varepsilon}{\sigma}\right)^n.
\end{equation*}
The full bound of Theorem~\ref{thm:prob_bound} is sharper in two ways: it vanishes as $\varepsilon\to 0$ (as it must), and for large $\sigma/\varepsilon$ it decays as $C_{\alpha,\beta,s,n}\cdot n(\alpha+\overline{\beta})\varepsilon/\sigma$. This $O(\varepsilon/\sigma)$ behaviour is essential for deriving the $O(1/t)$ tail bounds on condition numbers in Section~\ref{sec:robustness}.
\end{remark}
