...
\begin{proposition}\label{prop:pfaffian-degree-bound}
  ...
  \begin{equation*}
    \md(M) \le{} 2^{\frac{s(s-1)}{2}}(\alpha+\beta)^n\beta^c \cdot \left((n+\min\{n,s\})\alpha+(n+c)(\beta-1)+1\right)^s.
  \end{equation*}
  %\begin{align*}
...
\end{equation*}
and such that $\md_i(M) = \md(M,H)$. Clearly, the $i$-th maximal degree is coordinate-independent and
\begin{equation*}
  \md_i(M) = \md(M,H) = \md(\tilde{M},\tilde{H}),
\end{equation*}
