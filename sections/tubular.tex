...
\begin{equation*}
 H = \{x\in \R^n \colon h_{c+i+1}(x)=\cdots =h_{n}(x)=0\} 
\end{equation*}
It suffices to provide a uniform bound on $\md(M,H)$ for all such $H$. Clearly, the maximal degree is coordinate-independent and
\begin{equation*}
  \md(M,H) = \md(\tilde{M},\tilde{H}),
\end{equation*}
where $\tilde{M}=\{x\colon \tilde{f}_i(x)=f(h^{-1}(x))=0\}$ and $\tilde{H} = \{x\colon x_{c+i+1}=\cdots =x_{n}=0\}$. 
...
Since $M \setminus \partial M \subset V$ and $\dim M \setminus \partial M = \dim V$, we get $\md_i(M\setminus \partial M) \leq \md_i(V)$. The set $M\setminus\partial M = V\cap B^\circ(p,\sigma+\varepsilon)$ is a bounded smooth hypersurface without boundary, but it is not compact, so Theorem~\ref{thm:tube_bound} does not apply to it directly; we extend the bound by a compact exhaustion. By Sard's theorem applied to the map $x\mapsto \|x-p\|$ on $V$, almost every $r\in(0,\sigma+\varepsilon)$ is a regular value, so each $K_r:=V\cap \overline{B(p,r)}$ is a compact hypersurface. Fix regular values $r_k\nearrow\sigma+\varepsilon$, so that $K_{r_k}\nearrow M\setminus\partial M$. For each $k$, Theorem~\ref{thm:tube_bound} applied to the compact set $K_{r_k}\subset B(p,\sigma+\varepsilon)$, together with the monotonicity $\md_i(K_{r_k})\le\md_i(V)$, bounds $\vol T(K_{r_k},\varepsilon)$ by the right-hand side below, uniformly in $k$. Note that the $K_{r_k}$ are manifolds with boundary $V\cap S^{n-1}(p,r_k)$ but this does not affect the argument, as the bound of Theorem~\ref{thm:tube_bound} depends only on the section degrees $\md_i$, the codimension-$2$ boundary contributing only terms of the kind bounded for $\partial M$ below. Since the closed neighbourhoods $T(K_{r_k},\varepsilon)$ increase to $T(M\setminus\partial M,\varepsilon)$ up to a Lebesgue-null set, monotone convergence of measure gives
\begin{equation*}
...
\end{equation*}
Recalling $C_{\alpha,\beta,s,n}=6K$, $a=\alpha+\overline{\beta}$, and $t=\varepsilon/\sigma$ completes the proof.
\end{proof}
