...
\subsection{Generalised Gauss map}
 For $p\in M$, let $N_{p} M = \{ v \in \R^n \colon v \perp T_{p} M \}$ denote the normal space to $M$ at $p$ and  $S(NM)=\{(p,v)\in M\times S^{n-1}\colon v \perp T_{p} M\}$ the normal sphere bundle of $M$. The normal sphere bundle $S(NM)$ is a smooth manifold of dimension $n-1$ for any submanifold $M$, and is compact when $M$ is; in particular the generalised Gauss map is defined whether or not $M$ is compact.
...
\end{equation*}
where $\reg f$ is the set of regular values of $f$. For a general smooth map this supremum may be infinite; for the generalised Gauss maps of (sections of) Pfaffian sets to which we apply it, it is finite and attained---bounded by the degree estimates of this section---and the domain need not be compact.
...
\end{equation*}
The constant involves both $\beta$ and $\overline{\beta}=\max\{\beta,2\}$: the bare $\beta$ is the degree of $f$, while $\overline{\beta}$ is forced by the codimension-two boundary $V\cap S^{n-1}(p,\sigma+\varepsilon)$ arising in the proof, whose defining sphere has degree~$2$; the two coincide once $\beta\ge 2$.
\end{theorem}
...
Since $M \setminus \partial M \subset V$ and $\dim(M \setminus \partial M) = \dim V$, we have $\md_i(M\setminus \partial M) \leq \md_i(V)$. Applying Theorem~\ref{thm:tube_bound} to the compact manifold-with-boundary $M\subset B(p,\sigma+\varepsilon)$---which is legitimate by Remark~\ref{rem:tube_bound_boundary}, the bound being a one-sided integral-geometric estimate in the section degrees, with the $\md_i$ taken on the interior $M\setminus\partial M$---and using $\md_i(M\setminus\partial M)\le\md_i(V)$, we obtain
\begin{equation*}
...
\end{equation*}
Recalling $C_{\alpha,\beta,s,n}=6K$, $a=\alpha+\overline{\beta}$, and $t=\varepsilon/\sigma$ gives the claimed bound for every $\sigma$ such that $\sigma+\varepsilon$ is a regular value of $x\mapsto\|x-p\|$ on $V$, which by Sard's theorem is a dense set of $\sigma>0$. It remains to remove this genericity assumption. Both sides of the inequality are continuous in $\sigma$: the right-hand side is a polynomial in $\varepsilon/\sigma$, while the left-hand side equals $\vol\bigl(T(V,\varepsilon)\cap B(p,\sigma)\bigr)/(\omega_n\sigma^n)$, which is continuous because $\sigma\mapsto\vol\bigl(T(V,\varepsilon)\cap B(p,\sigma)\bigr)$ is (the spheres $S^{n-1}(p,\sigma)$ being Lebesgue-null). Since the bound holds on a dense set of $\sigma$ and both sides are continuous, it holds for all $\sigma,\varepsilon>0$, completing the proof.
\end{proof}
