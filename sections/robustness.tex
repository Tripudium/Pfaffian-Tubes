...
\pl{To get the $O(1/t)$ decay for large $t$, don't we need to use the bound from Corollary 3.6 here?}
\end{remark}
...
For single-hidden-layer sigmoid networks, the BKK-based tube bound of Theorem~\ref{thm:prob_bound_single_layer} yields a sharper tail bound in which the Khovanskii prefactor $2^{h(h-1)/2}$ of Corollary~\ref{cor:nn-condition} is replaced by a polynomial in the width.
